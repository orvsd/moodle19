...
if ($action == 'updatestatus') {
	...
	if($CFG->ilptarget_send_target_message == 1){
			...
			$targeturl = $CFG->wwwroot.'/mod/ilptarget/target_view.php?'.(($courseid)?'courseid='.$courseid.'&amp;' : '').'userid='.$id.'&amp;status='.$status;
            $targetview = get_string('targetviewlink','ilptarget');
			$updatedstatus = get_string('statusupdate', 'ilptarget', $thistargetstatus);

			$message = '<p>'.$updatedstatus.'<br /><a href="'.$targeturl.'">'.$targetview.'</a></p>';
