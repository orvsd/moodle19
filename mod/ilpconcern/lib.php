...
function update_student_status_menu($userid,$courseid) {
	...
	if(!$post = get_record('ilpconcern_status', 'userid',$user->id)) {
		...
		$options = array(get_string('green', 'ilpconcern'), get_string('amber', 'ilpconcern'), get_string('red', 'ilpconcern'), get_string('silver', 'ilpconcern'), get_string('gold', 'ilpconcern'));
...
class ilpconcern_updateconcern_form extends moodleform {
    ...
    function definition() {
		...
		if($concernspost > 0 && $report){
			...
			if($CFG->ilpconcern_use_template == 1 && $template > 0){
				...
				$mform->addElement('hidden', 'template', $thistemplate->id);
		...
		$statusoptions = array(get_string('green', 'ilpconcern'), get_string('amber', 'ilpconcern'), get_string('red', 'ilpconcern'), get_string('silver', 'ilpconcern'), get_string('gold', 'ilpconcern'));
		
		if(eval('return $CFG->ilpconcern_report'.($status + 1).'_status;') == 1) {
			$mform->addElement('select', 'rec_status', get_string('rec_status', 'ilpconcern'), $statusoptions);
			if($concernspost > 0 && $report){
				$mform->setDefault('rec_status', $report->rec_status);
			}
		}else{
			$mform->addElement('hidden', 'rec_status', -1);
		}
	...
	function process_data($data) {
		...
		if (!$report = get_record('ilpconcern_posts', 'id', $concernspost)) {
			...
			$report->rec_status = $data->rec_status;
