...
.addbox a.button, .ilpadd a.button {
    background: transparent url('<?php echo $CFG->wwwroot; ?>/blocks/ilp/pix/addbox_a.gif') no-repeat scroll top right;
    color: #444;
    display: block;
    float: left;
...
.addbox a.button span, .ilpadd a.button span {
    background: transparent url('<?php echo $CFG->wwwroot; ?>/blocks/ilp/pix/addbox_span.gif') no-repeat;
    display: block;
    line-height: 14px;
...
h1.status-3, h2.status-3, h3.status-3, span.status-3 { color: #8f8fbd; }
h1.status-4, h2.status-4, h3.status-4, span.status-4 { color: #CD7F32; }
span.status-0, span.status-1, span.status-2, span.status-3, span.status-4 { font-weight:bold; }
h2.status-0, h2.status-1, h2.status-2, h2.status-3, h2.status-4 { margin: 10px; padding: 0px; line-height: 1.3em; text-align: center; font-size: 1.4em; }

/* Use full-size profile image */
#blocks-ilp-view .userpicture { width: 100px; height: 100px; }

/* Target Category Key */
div.target-category-key {border: 1px solid #ccc; margin:auto; width:800px}
div.target-category-key p {float: left}

/* Progress Bars for Attendance and Progress Display */
...
div.progress-container span a {
 color: #000;
}

.attendance-green { background-color:#99FF99; }
.attendance-amber { background-color:#FFD966; }
.attendance-red { background-color:#FF6666; }
.attendance-blue { background-color:#B0E0E6; }

/* Target Category Backgrounds - add as appropriate with corresponding ID number */
.target-category-1 {background:#ccddcc;}
.target-category-2 {background:#eeffee;}
.target-category-3 {background:#ccddff;}
.target-category-4 {background:#ddddcc;}
.target-category-5 {background:#eeeecc;}
.target-category-6 {background:#ffdddd;}
